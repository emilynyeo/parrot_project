...
meta <- meta1 %>%
  full_join(meta2) %>%
  mutate(Given.probiotic= ifelse(is.na(Given.probiotic), "", ifelse(as.character(Given.probiotic)=="Yes", "Given probiotics", "")))
...
# Remove empty rows
otu16m_df <- otu16m_df[rowSums(otu16m_df[,-1])>0,]
otu16n_df <- otu16n_df[rowSums(otu16n_df[,-1])>0,]
...
# Remove empty rows
otu16ren_df <- otu16ren_df[rowSums(otu16ren_df[,-1])>0,]

colnames(reads_18ren) <- gsub("^s0","S0", colnames(reads_18ren))
otu18ren_df <- reads_18ren %>%
  select(ESVId, starts_with("S1") & ends_with(".1"))
# Remove empty rows
otu18ren_df <- otu18ren_df[rowSums(otu18ren_df[,-1])>0,]
...
ReadDepth18ren <- colSums(otu18ren_mat) %>% as.data.frame() %>%
  rownames_to_column(var="SampleId") %>%
  rename(ReadDepth18n=".")
meta_adj <- meta_adj %>% left_join(full_join(full_join(full_join(ReadDepth16m,ReadDepth18m), full_join(ReadDepth16n, ReadDepth18n)), full_join(ReadDepth16ren, ReadDepth18ren))) %>%
  mutate(Captive.Wild = factor(Captive.Wild, levels=c("Captive","Wild, free ranging", "Wild, seized from traffickers", "Crate 1: Unknown origin", "Crate 2: Unknown origin")))
...
reformatDates <- as.data.frame(matrix(nrow=length(meta_adj$Date.collected), ncol=5, dimnames = list(NULL, c("Date.collected","Day","Month","Year","DateAdj"))))
...
meta_adj <- meta_adj %>%
  mutate(TimeSinceSeizure = as.Date(DateAdj)-Seized.date)
...
#### Get taxonomic assignments (from Jonah) ####
...
meta_adj %>% filter(SampleId %in% colnames(otu16n_mat_filt2)[which(colSums(otu16n_mat_filt2)<cutoff16Snano)]) %>%
  pull(SampleId)
which(log10(colSums(otu18n_mat_filt2))<2)
