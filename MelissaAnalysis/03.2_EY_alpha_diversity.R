...
#### Colour legends ####
captivewild_legend <- c(Captive="purple4", 
...
location_legend <- c(Nigeria ="goldenrod4",
                     `South Africa`="darkred", 
                     Unknown="grey")
...
# All samples
alpha_16 <- plot_richness(phyloseq16nano, 
                          measures = c("Chao1","Shannon","InvSimpson"))$data%>%
...
alpha16n_stats <- compare_means(value_adj ~ Captive.Wild, data = alpha_16 %>%   
                                  filter(Captive.Wild %in% 
                                           c("Captive", "Wild, free ranging",
                                             "Wild, seized from traffickers")), 
                                p.adjust.method="fdr", 
                                method="t.test", group.by = "variable_adj") %>%
...
gg_alpha16n_sourceprobiotic_wstats <- alpha_16 %>%
  ...
  geom_point(aes(group=Given.probiotic, 
                 pch=Given.probiotic),
